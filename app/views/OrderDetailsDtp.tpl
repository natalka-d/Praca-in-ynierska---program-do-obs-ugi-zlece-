...
<section class="admin-panel-wrapper">
    ...
    <div class="admin-header">
        <h2>🎨 Szczegóły zlecenia (Studio DTP)</h2>
    </div>
    ...
    <div class="admin-panel">
        ...
        <!-- Etap DTP -->
        <h2>Etap Studio DTP</h2>
        <section class="card stage-card toggle-card" data-stage="{$order.stage_id}">
            <div class="stage-summary">
                <p><strong>Opis:</strong> {$order.dtp_description|default:"Brak opisu"}</p>
                <p>
                    <strong>Status:</strong>
                    {if ($order.done|default:0) == 1}
                        <span class="status-badge status-done">✅ Zakończone</span>
                        <a href="{url action='order_toggle_dtp' stage_id=$order.stage_id id=$order.id}" class="button button-revert">↩ Cofnij</a>
                    {else}
                        <span class="status-badge status-progress">⌛ W trakcie</span>
                        <a href="{url action='order_toggle_dtp' stage_id=$order.stage_id id=$order.id}" class="button button-complete">✔ Oznacz jako skończone</a>
                    {/if}
                </p>
            </div>
        </section>
        ...
        <!-- Powrót -->
        <div class="back-link">
            <a href="{$conf->action_root}orders_dtp" class="button">⬅ Powrót do listy</a>
        </div>
