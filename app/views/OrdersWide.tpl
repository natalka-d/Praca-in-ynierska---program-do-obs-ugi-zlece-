...
<section class="wrapper orders-admin"> {* taka sama klasa jak u drukarza *}

    <!-- Nagłówek panelu -->
    <div class="header-panel">
        <h2>📐 Panel Druku Wielkoformatowego</h2>
    ...
    {if isset($orders) && $orders|count > 0}
                    ...
                    {foreach $orders as $order}
                        <tr>
                            ...
                            <td data-label="Status">
                                ...
                                {elseif $order.status|default:"" == "w trakcie"}
                            ...
                            <td class="actions-cell" data-label="Akcje">
                                <a href="{$conf->action_root}order_details_wide/{$order.id}" class="action-link">🔍 Szczegóły</a>
                            </td>
                        </tr>
                    {/foreach}
                </tbody>
        ...
        {if $total_pages > 1}
            <div class="pagination">
                {if $current_page > 1}
                    <a href="{$conf->action_root}orders_wide/{$current_page-1}" class="page-link">⬅ Poprzednia</a>
                {/if}
                <span class="current-page">Strona {$current_page} z {$total_pages}</span>
                <span style="color:#666;">(Łącznie zleceń: {$total_orders})</span>
                {if $current_page < $total_pages}
                    <a href="{$conf->action_root}orders_wide/{$current_page+1}" class="page-link">Następna ➡</a>
                {/if}
            </div>
        {/if}
